...
```{r canonincal, fig.cap="The canonincal case", warning = FALSE}
...
Staggered adoption refers to the situation where units receive the treatment at different points in time. Staggered adoption does not cause any problem for the DiD-estimator/TWFE if the treatment effect is homogeneous and instantaneous (see [**Goodman-Bacon**](problems.html)).

Figure \@ref(fig:staggered) illustrates a situation with 3 units and staggered adoption of treatment.
...
The potential problems with the standard TWFE approach can be summarized in 2 points:

1. Units receive negative weights

2. Rely on parametric assumptions


Strictly speaking, only the issue of negative weights is a problem for inference. Parametric assumptions are just less convenient.

Most contributions offer solutions to adapt the DiD-estimator to these problems, but [**Wooldridge**](solutions.html) offers solutions using TWFE-models.
